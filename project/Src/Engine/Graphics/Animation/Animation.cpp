...
Vector3 CalculateValue(const std::vector<KeyframeVec3>& keyframes, float time) {
	...
	for (size_t index = 0; index < keyframes.size() - 1; ++index) {
		...
		if (keyframes[index].time <= time && time <= keyframes[nextIndex].time) {
			...
			return Lerp(keyframes[index].value, keyframes[nextIndex].value, t); // Vector3の線形補間
		}
	}
...
Quaternion CalculateValue(const std::vector<KeyframeQuaternion>& keyframes, float time) {
	...
	for (size_t index = 0; index < keyframes.size() - 1; ++index) {
		...
		if (keyframes[index].time <= time && time <= keyframes[nextIndex].time) {
			float t = (time - keyframes[index].time) / (keyframes[nextIndex].time - keyframes[index].time);
			return Slerp(keyframes[index].value, keyframes[nextIndex].value, t); // Quaternionの球面線形補間
		}
	}
	return (*keyframes.rbegin()).value;
}
